...
class DashboardView extends React.Component {
  ...
  componentDidMount = () => {
    ...
    this.notificationSubscription = Notifications.addListener(this.handleNotification);
     setInterval(this.getPanicVideoAsync, 5000);
  };
  ...
  registerForPushNotificationsAsync = async () => {
    ...
    let res = await axios
      .post(`${API_HOST}/push/${token}`, { "token": token, "user": this.props.userData })
      .catch(err => console.log(err));
  };
  ...
  getPanicVideoAsync = async () => {
    let panicVideo = await axios.get(`${API_HOST}/panic/anyMember/${this.props.userData.id}`).catch(err => console.log(err));
   if (!panicVideo.data[0]) {
     console.log('did not exist');
   }
   else {
     this.setState({ panicVideoUrl: panicVideo.data[0].url_video});
     console.log(this.state.panicVideoUrl);
     this.props.navigation.navigate('PanicVideoView', {
       panicVideoUrl: this.state.panicVideoUrl,
     })
   }
  };
